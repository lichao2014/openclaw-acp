...
- 支持 Gateway 协议 `v3` 和 `v4`，默认使用 `auto` 自动选择。
...
```bash
npx -y @lichao2014/openclaw-acp --config-path ~/.openclaw
```
...
```bash
npx -y @lichao2014/openclaw-acp --gateway-protocol v4
```
...
```json
{
  "agents": {
    "openclaw-gateway": {
      "command": "npx",
      "args": ["-y", "@lichao2014/openclaw-acp", "--config-path", "./openclaw-state"]
    }
  }
}
```
...
```text
openclaw-acp [--config-path <dir>] [--gateway-protocol auto|v3|v4]
openclaw-acp --url <gateway-url> --token <gateway-token> [--config-path <dir>] [--gateway-protocol auto|v3|v4]
```

| 参数 | 说明 |
| --- | --- |
| `--config-path <dir>` | 包含 `openclaw.json` 的目录。默认是 `~/.openclaw`。兼容旧写法 `--config_path`。 |
| `--url <gateway-url>` | Gateway WebSocket 地址，必须和 `--token` 一起使用。 |
| `--token <gateway-token>` | Gateway 认证 token，必须和 `--url` 一起使用。 |
| `--gateway-protocol auto|v3|v4` | Gateway 协议选择，默认是 `auto`。兼容旧写法 `--gateway_protocol`。 |

`auto` 模式下，适配器会先按 Gateway 协议 `v4` 建立连接；如果 Gateway 返回
`protocol mismatch`，再自动回退到 `v3`。这个判断基于当前正在运行的 Gateway
握手结果，不依赖 `openclaw.json` 里的配置写入版本。
